...
function Header ( {
    ...
    function createLis () {
        ...
        const lis = chapters[ language ].map( item => (
            <li
                key={ item }
            ...
            lis.push( <li key="icon" className={ "icons" }><Icon languages={ languages } setLanguage={ setLanguage }/></li> );
    ...
    function handleClick ( event ) {
        ...
        if ( event.target.className === "icons" ) return;

        switch ( event.target.textContent ) {

            case chapters[ language ][ 0 ]:
                setChapter( chapters[ language ][ 0 ] );
                break;

            case chapters[ language ][ 1 ]:
                setChapter( chapters[ language ][ 1 ] );
                break;

            case chapters[ language ][ 2 ]:
                setChapter( chapters[ language ][ 2 ] );
                break;
...
function Icon ( { languages, setLanguage } ) {

    return (
        <>
            <GithubIcon/>
            <LanguageIcon languages={ languages } setLanguage={ setLanguage }/>
            <ThemeIcon/>
        </>
    );
...
function LanguageIcon ( { languages, setLanguage } ) {
    ...
    function handleAsideClick ( event ) {
        ...
        if ( event.target.nodeName.toLowerCase() !== "p"  ) return;

        const content = event.target.textContent;

        setLanguage( languages[ content === "English" ? 0 : 1 ] );
        setVisible( false );
