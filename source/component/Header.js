...
function Header ( {
    ...
    return (
        <header>
            <h1>font-caster</h1>
            <ul onClick={ handleClick }>{ createLis() }</ul>
        </header>
    );

    function createLis () {

        const lis = chapters.map( item => (
            <li
                key={ item }
                className={ item === chapter ? "selected" : "non-selected" }
            >{ item }</li>
        ) );

            lis.push( <li key="icon"><Icon/></li> );
